...
def to_local_time_string(dt):
    """Convert UTC datetime to local time string in a consistent format"""
    if not dt:
        return None
...
class Vehicle(Base):
    ...
    waitlist_entry = relationship(
        "WaitlistEntry", back_populates="vehicle", uselist=False
    )
    ...
    def to_dict(self):
        return {
            ...
            "arrival_time": to_local_time_string(self.arrival_time),
            "estimated_departure": to_local_time_string(self.estimated_departure),
            "actual_departure": to_local_time_string(self.actual_departure),
            "is_vip": self.is_vip,
            ...
            "status": (
                "parked"
                if self.slot_id
                else "waitlisted" if self.waitlist_entry else "departed"
            ),
        }
